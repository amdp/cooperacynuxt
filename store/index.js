...
export const state = () => ({
  ...
  category: [
    ...
    { id: 2, name: 'Community' },
    {
      id: 3,
      name:
        'Ecosystem and Wellbeing - Must really be related with the ecosystem care or with human wellbeing'
    },
    { id: 4, name: 'Reporting' },
    {
      id: 5,
      name: 'Location-based - For official cities, nations, neighbourhoods'
    },
    {
      id: 6,
      name: 'Science, Research, Education and Professionals groups and projects'
    },
...
export const actions = {
  ...
  placeFormAction: async (context, payload) => {
    let { data } = await axios.post(process.env.DBURL + '/place', payload);
  ...
  recoverPasswordAction: async (context, payload) => {
    axios.post(process.env.DBURL + '/recoverpassword', payload);
  },
  ...
  resetcpVotingAction: async (context, payload) => {
    let { data } = await axios.post(process.env.DBURL + '/resetcpvoting');
    return 'OK';
  },
  resetuVotingAction: async (context, payload) => {
    let { data } = await axios.post(process.env.DBURL + '/resetuvoting');
    return 'OK';
  }
};
