...
var pvotesModel = db.sequelize.define(
  'pvote',
  {
      id: {type: Sequelize.INTEGER, primaryKey: true, autoincrement: true},
...
cooperacyRouter.post("/projects", (req, res) => { if(!req.body.name){ 
    res.status(400); res.json({ error: "Bad data" })} else { projectModel.create(req.body) 
      .then(() => { res.send("Project added") }) .catch(err => { res.send("Error: " + err) })
  }
})
...
cooperacyRouter.get('/user', (req, res, next) => { res.json({ user: req.user }) })
...
cooperacyRouter.get('/pvotes', (req, res) => { pvotesModel.findAll ()
