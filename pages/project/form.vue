  ...
  <div class="row">
    ...
    <div class="col-8">
      ...
      >
        ...
        <p>If you do not find your place, please feel free to add a new <b-link
            v-b-modal.placemodal
            class="ac"
          >one</b-link>
        </p>
        <b-form-group
        ...
        >
          ...
          <div class="col-2"> Current: <br><img
              class="currentImage"
              :src="projectImage"
            /></div>
        </div>
        ...
        <h6
          class="trust"
          v-if="this.totallabel != -1"
        >TOTAL NON-FUNDED BUDGET: {{totallabel}}</h6>
        <b-button
          type="submit"
          class="btn btrust btn-block mt-3 gray border-0"
        >GO!</b-button>
      </b-form>
...
export default {
  ...
  computed: {
    ...
    place() {
      let place = this.$store.state.place.filter(place => {
        return place.country === this.formCountry;
      });
  ...
  data() {
    return {
      ...
      category: this.$store.state.category.filter(category => {
  ...
  methods: {
    ...
    addplace() {
      let result = this.$store.dispatch("placeFormAction", {
