...
<template>
        <b-row class="p-0 w-100 m-0" v-if="this.proptype=='user'"><b-col cols="12" class="p-0 m-0 w-100 d-flex">
...
export default {
  ...
  computed: {
    vote(){//need to avoid this to cycle through user votebar, which have always the same color
        let voteif = []; for (let i=0;i<7;i++){
          ...
          this.$store.state[this.proptype + 'uservote'] ? uservoted = this.$store.state[this.proptype + 'uservote'].findIndex(
            x => x[this.proptype] == this.voteprop.id && x.condition == this.$store.state.cc[i])
            : uservoted = 0
      ...
      if (this.proptype == 'user') { for (let i=0;i<7;i++){
        ...
        voteif[i].style = 'width: '+this.$auth.user[this.$store.state.cc[i]]/28*100 +'%',
  ...
  methods: {
    voteswitch(cc){ 
      ...
      this.$store.commit('setVoteUpdate', {id: this.voteprop.id, cc: cc, user: this.$auth.user.id, add: add, proptype: this.proptype})
      ...
      if (this.proptype == 'comment'){request.projectid=this.$route.params.id;request.author=this.voteprop.user}
